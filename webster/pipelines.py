...
class WebsterPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `webster`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM webster WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO webster (date, title, link, content)"
